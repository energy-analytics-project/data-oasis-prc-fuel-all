...
DATABASE        = "caiso-oasis-prc-fuel-all.db"
...
# resource name
# -----------------------------------------------------------------------------
RESOURCE_NAME="data-oasis-prc-fuel-all"
...
#    [pp.pprint(r) for r in parse(new_xml_files()[:1])]
    try:
        ...
        xml_files   = new_xml_files()
        reports     = parse(xml_files)
        inserted    = []
        for report in reports:
            inserted += insert(reports, cnx)
        update_manifest(inserted)
    except:
        cnx.close()
